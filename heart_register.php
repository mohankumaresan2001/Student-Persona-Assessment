...
$uname = $password = $cpassword = $sname = $email = $tsname = $temail= $dob = $batch_code = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  ...
  // Check input errors before inserting in database
	if(empty($uname_err) && empty($password_err) && empty($cpassword_err) && empty($aname_err) && empty($email_err) && empty($dob_err) && empty($batchcode_err))
	{
		...
		$sql = "INSERT INTO heart_login (user_name, staff_name, password, batch_code, email, dob) VALUES (?, ?, ?, ?, ?, ?)";
		
		if($stmt = mysqli_prepare($link, $sql))
		{
            // Bind variables to the prepared statement as parameters
			mysqli_stmt_bind_param($stmt, "sssiss", $param_username, $param_aname ,$param_password, $param_batchcode, $param_email, $param_dob);
